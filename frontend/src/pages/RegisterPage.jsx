...
export default function RegisterPage() {
  ...
  async function onSubmit(values) {
    setTaken(null);
    try {
      const data = await registerPlayer(values);
      if (data.pending) {
      ...
      if (data?.code === 'request_pending') {
        toast.error(data.error);
        return;
      }
      toast.error(data?.error || 'Falha ao enviar a solicitação');
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gulag-bg flex items-center justify-center px-4 py-10">
      <div className="max-w-sm w-full">
        ...
        <h1 className="text-2xl font-bold mb-2 text-center text-gulag-cyan tracking-wide">Solicitar cadastro</h1>

        {sent ? (
          <div className="rounded border border-emerald-700/60 bg-emerald-500/10 p-4 text-center">
            <p className="text-emerald-300">{sent}</p>
            <Link to="/login" className="text-sm text-gulag-cyan underline mt-3 inline-block">
              Voltar para o login
            </Link>
          </div>
        ) : (
          <>
            <p className="text-sm text-gray-400 text-center mb-6">
              Preencha seus dados abaixo. Um administrador vai revisar e aprovar seu cadastro antes de você
              conseguir entrar.
            </p>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-3">
              ...
              <Button disabled={formState.isSubmitting}>Enviar solicitação</Button>
            </form>
